...
# REPLACE esummary_titles_async() with this optimized version:

async def esummary_titles_async(session: aiohttp.ClientSession, pmids: List[str]) -> Dict[str, str]:
    """Optimized title fetching with better parallelization"""
    titles: Dict[str, str] = {}
    if not pmids:
        return titles
    ...
    # SMALLER batches processed in PARALLEL instead of large sequential batches
    BATCH = 300  # Reduced from 200 for better balance
    ...
    # MUCH MORE PARALLEL: Process multiple batches simultaneously
    semaphore = asyncio.Semaphore(6)  # Increased from 1 (sequential) to 6
    
    async def controlled_fetch(batch):
        async with semaphore:
            result = await fetch_single_batch(batch)
            await asyncio.sleep(RATE_LIMIT_SEC * 0.3)  # Reduced delay
            return result
...
async def fetch_abstracts_batch(session: aiohttp.ClientSession, pmids: List[str]) -> Dict[str, Dict]:
    ...
    async def controlled_fetch(batch):
        async with semaphore:
            result = await fetch_single_batch(batch)                    # CHANGE #3: 
            await asyncio.sleep(RATE_LIMIT_SEC * 0.2)  # Added delay    # CHANGE #3:
            return result                                               # CHANGE #3:
